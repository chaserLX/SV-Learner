...
def cal_support_vectors(Lx,net2,inputs_x_w,targets_x,args,batch_size):
    ...
    # # select index for unlabeled samples which >= threshold
    # idx_confident = Lu.nonzero(as_tuple=False).squeeze()
    #
    # if len(idx_confident) > 0:
    #
    #     # select losses for unlabeled samples which >= threshold
    #     L = Lu.clone().detach()
    #     loss = L[idx_confident]
    #
    #     losses = torch.zeros(len(idx_confident))
    #     for b in range(len(idx_confident)):
    #         losses[b] = loss[b]
    #     select_losses = (losses - losses.min()) / (losses.max() - losses.min())
    #     select_losses = select_losses.reshape(-1, 1)
    #
    #     # fit a two-component GMM to the loss
    #     gmm = GaussianMixture(n_components=2, max_iter=10, tol=1e-2, reg_covar=5e-4)
    #     gmm.fit(select_losses)
    #     prob = gmm.predict_proba(select_losses)
    #     prob = prob[:, gmm.means_.argmin()]
    #
    #     # mid index: index in selected array
    #     select_mid_index = (prob >= args.flex_threshold).nonzero()[0]
    #
    #     select_prob = prob[select_mid_index]
    #     select_prob = torch.Tensor(select_prob)
    #     # last index: index in original array
    #     select_last_idx_u = np.array(idx_confident.cpu())[select_mid_index]
    #
    #     # feat of svmtripet contrastive learning samples
    #     select_con_u_w2, _ = net2(inputs_u_w[select_last_idx_u])
    #     select_targets_u = targets_u[select_last_idx_u]

        # support_vector_s = []
        # support_label_s = []
        #
        # # find label->images_index
        # images_select_lists_s = [[] for i in range(args.num_class)]
        # for i in range(len(select_last_idx_u)):
        #     images_select_lists_s[select_targets_u[i]].append(i)
        #
        # # find support_vector_features
        # for j in range(args.num_class):
        #     if images_select_lists_s[j]:
        #         s_prob = select_prob[images_select_lists_s[j]].unsqueeze(0)
        #
        #         median_prob = torch.median(s_prob)
        #
        #         sup_index = torch.nonzero(s_prob == median_prob)[0][1]
        #
        #         index = images_select_lists_s[j][sup_index]
        #         support_vector_s.append(select_con_u_w2[index].unsqueeze(0))
        #
        #         support_label_s.append(j)
        #     else:
        #         pass
...
# def cal_svmtripet_loss_mixhc(support_vectors_h, support_vectors_s, pred_norm, target_norm, p, target_a, target_b, support_labels_h, criterion_triplet, support_labels_s, l):
#
#     n = pred_norm.size(0)
#     dist = -torch.matmul(pred_norm, target_norm.t())
#
...
#
#         # dist_ap.append(dist[i][mask[i]].max().unsqueeze(0))
#         pos_label_a = target_a[i]
#         pos_label_b = target_b[i]
#
#         if support_vectors_h.size(0) < p['num_classes']:
#             # neg_class_id = torch.Tensor(neg_class_id)
#             # neg_class_id = int(neg_class_id[0])
#
#             support_labels_h = np.array(support_labels_h)
#             if int(pos_label_a) in support_labels_h:
#                 index_pos = np.nonzero(support_labels_h == int(pos_label_a))
#                 # dist_ap.append(dist_supcon[i][index_pos])
#                 dist_ap.append(dist_sup_h[i][index_pos])
#             else:
#                 dist_ap.append(dist[i][mask[i]].max().unsqueeze(0))
#
#             if int(pos_label_b) in support_labels_h:
#                 index_pos_b = np.nonzero(support_labels_h == int(pos_label_b))
#                 # dist_ap.append(dist_supcon[i][index_pos])
#                 dist_ap_b.append(dist_sup_h[i][index_pos_b])
#             else:
#                 dist_ap_b.append(dist[i][mask[i]].max().unsqueeze(0))
#
#         else:
#             support_labels_h = np.array(support_labels_h)
#             index_pos = np.nonzero(support_labels_h == pos_label_a.item())
#             dist_ap.append(dist_sup_h[i][index_pos])
#
#             index_pos_b = np.nonzero(support_labels_h == int(pos_label_b))
#             dist_ap_b.append(dist_sup_h[i][index_pos_b])
...
#
#         neg_class_ids = set(support_labels_s)
#         if int(pos_label_a) in neg_class_ids:
#             neg_class_ids.remove(int(pos_label_a))
#
#         if int(pos_label_b) in neg_class_ids:
#             neg_class_ids.remove(int(pos_label_b))
#
#         neg_class_id = sample(neg_class_ids, 1)
#
#         support_labels_s = np.array(support_labels_s)
...
#
#     dist_ap = torch.cat(dist_ap)
#     dist_ap_b = torch.cat(dist_ap_b)
#     dist_an = torch.cat(dist_an)
#     y = torch.ones_like(dist_an)
#
#     loss_triplet_a = criterion_triplet(dist_an, p['gamma'] * dist_ap, y)
#
#     loss_triplet_b = criterion_triplet(dist_an, p['gamma'] * dist_ap_b, y)
#
#     loss_triplet = l * loss_triplet_a + (1 - l) * loss_triplet_b
#
#     return loss_triplet

def svmfix_train(args, epoch, net, net2, optimizer, labeled_trainloader,unlabeled_trainloader, criterion_triplet, classwise_acc, contrastive_criterion):
    ...
    for batch_idx, (inputs_x, inputs_x2, inputs_x3, inputs_x4, labels_x, w_x, index_x) in enumerate(labeled_trainloader):
        ...
        with torch.no_grad():
            ...
            if epoch < args.warm_up + 2:
                mask = max_probs.ge(args.flex_threshold).long()
            else:
                mask = max_probs.ge(
            ...
            # select_x = max_probs_x.ge(args.flex_threshold).long()
        ...
        if epoch >= args.epoch_start_svmfix:
            ...
            ## Total Loss
            loss = Lx + lambda_u * Lu + args.lambda_c * loss_simCLR + args.lambda_tri * L_hs + penalty
            loss_tri += L_hs.item()
        else:
        ...
        sys.stdout.write('\r')
        sys.stdout.write(
            '%s:%.1f-%s | Epoch [%3d/%3d] Iter[%3d/%3d]\t Labeled loss: %.2f  Unlabeled loss: %.4f SimClR Loss:%.4f svmtri Loss:%.4f'
    ...
    if max(pseudo_counter.values()) < len(unlabeled_trainloader.dataset):  # not all(5w) -1
        for i in range(args.num_class):
            ...
            if classwise_acc_next[i] < 0.5:
                classwise_acc_next[i] = 0.5
    ...
    #
    #         if classwise_acc_next[i] < 0.8:
    #             class_flex_adjust[i] = 0.8

    return classwise_acc_next
...
def svmfix_train_flex(args, epoch, net, net2, optimizer, labeled_trainloader,unlabeled_trainloader, criterion_triplet, classwise_acc, contrastive_criterion):
    ...
    for batch_idx, (inputs_x, inputs_x2, inputs_x3, inputs_x4, labels_x, w_x, index_x) in enumerate(labeled_trainloader):
        ...
        with torch.no_grad():
            ...
            # value = torch.mean(max_probs_true)
            # print("value", value)
            # select_x = max_probs_true.ge(0.6).long()
        ...
        sys.stdout.write('\r')
        sys.stdout.write(
            '%s:%.2f-%s | Epoch [%3d/%3d] Iter[%3d/%3d]\t Lb loss: %.2f  Ulb loss: %.4f SimClR Loss:%.4f svmtri:%.4f'
            % (args.dataset, args.r, args.noise_mode, epoch, args.num_epochs, batch_idx + 1, num_iter,
               loss_x / (batch_idx + 1), loss_u / (batch_idx + 1), loss_ucl / (batch_idx + 1), loss_tri / (batch_idx + 1)))
        sys.stdout.flush()
    ...
    if max(pseudo_counter.values()) < len(unlabeled_trainloader.dataset):  # not all(5w) -1
        for i in range(args.num_class):
            ...
            if classwise_acc_next[i] < args.flex_threshold:
    ...
    if max(pseudo_counter_x.values()) < len(labeled_trainloader.dataset):  # not all(5w) -1
        ...
        median_confidence = class_flex_adjust.median()
        for i in range(args.num_class):
            if class_flex_adjust[i] < 0.9:
                class_flex_adjust[i] = 1
    return classwise_acc_next, class_flex_adjust, median_confidence
