...
def svmfix_train_webvision(args, epoch, net, net2, optimizer, labeled_trainloader,unlabeled_trainloader, criterion_triplet, classwise_acc, contrastive_criterion):
    ...
    for batch_idx, (inputs_x, inputs_x2, inputs_x3, inputs_x4, labels_x, w_x, index_x) in enumerate(labeled_trainloader):
        ...
        if epoch >= args.epoch_start_svmfix:
            ...
            loss = Lx + penalty + args.lambda_tri * L_hs
            loss_tri += L_hs.item()
        else:
            # Unsupervised Contrastive Loss
            f1, _ = net(inputs_u3)
