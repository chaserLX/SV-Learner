...
class cifar_dataset(Dataset): 
    def __init__(self, dataset, sample_ratio, r, noise_mode, root_dir, transform, mode, noise_file='', psl=None, probability=[], log='', class_flex_adjust=[], gmm_prob=[]):
        ...
        if self.mode=='test':
            ...
            if psl is not None:
                if isinstance(psl, np.ndarray):
                    self.noise_label = psl.tolist()
                else:
                    self.noise_label = psl  # list
            else:
                self.noise_label = noise_label

            if self.mode == 'all' or self.mode == 'relabel':
                ...
                if self.mode == "labeled":
                    ...
                    # both GMM and JS Selection
                    samples = 0
                    for i in range(num_class):
                        samples += int(self.sample_ratio / class_flex_adjust[i] * len(self.class_ind[i]))
                        # samples += int(self.sample_ratio * len(self.class_ind[i]))
                    ...
                    # GMM
                    # self.probability = [gmm_prob[i] for i in pred_idx]
                    # JS
...
class cifar_dataloader():  
    ...
    def run(self, sample_ratio, mode, psl=[], prob=[], class_flex_adjust=[], gmm_prob=[]):
        if mode=='warmup':
            ...
            labeled_dataset = cifar_dataset(dataset=self.dataset, sample_ratio= sample_ratio, noise_mode=self.noise_mode, r=self.r, root_dir=self.root_dir, transform=self.transforms["labeled"], mode="labeled", noise_file=self.noise_file, psl=psl, probability=prob,log=self.log, class_flex_adjust=class_flex_adjust, gmm_prob=gmm_prob)
            ...
            unlabeled_dataset = cifar_dataset(dataset=self.dataset, sample_ratio= sample_ratio, noise_mode=self.noise_mode, r=self.r, root_dir=self.root_dir, transform=self.transforms["unlabeled"], mode="unlabeled", noise_file=self.noise_file, psl=psl)
            ...
            eval_dataset = cifar_dataset(dataset=self.dataset, sample_ratio= sample_ratio, noise_mode=self.noise_mode, r=self.r, root_dir=self.root_dir, transform=self.transform_test, mode='all', noise_file=self.noise_file, psl=psl)
            ...
            relabel_dataset = cifar_dataset(dataset=self.dataset, sample_ratio=sample_ratio, noise_mode=self.noise_mode, r=self.r, root_dir=self.root_dir, transform=self.transform_test, mode='relabel', noise_file=self.noise_file)
