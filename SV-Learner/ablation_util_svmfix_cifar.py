...
def svmfix_train_flex(args, epoch, net, net2, optimizer, labeled_trainloader,unlabeled_trainloader, criterion_triplet, classwise_acc, contrastive_criterion):
    ...
    if max(pseudo_counter_x.values()) < len(labeled_trainloader.dataset):  # not all(5w) -1
        for i in range(args.num_class):
            class_flex_adjust[i] = pseudo_counter_x[i] / max(pseudo_counter_x.values())
            gmm_cof += class_flex_adjust[i]
            if class_flex_adjust[i] < 0.9:
                class_flex_adjust[i] = 0.9
        gmm_cof = gmm_cof / args.num_class
    return classwise_acc_next, class_flex_adjust, gmm_cof
