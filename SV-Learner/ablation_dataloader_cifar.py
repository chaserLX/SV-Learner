...
class cifar_dataset(Dataset): 
    def __init__(self, dataset, sample_ratio, r, noise_mode, root_dir, transform, mode, noise_file='', pred=[], probability=[], log='', class_flex_adjust=[], gmm_prob=[]):
        ...
        if self.mode=='test':
            ...
            if self.mode == 'all':
                ...
                if self.mode == "labeled":
                    ...
                    # for i in range(num_class):
                    #     class_indices = self.class_ind[i]
                    #     prob1 = np.argsort(-1 * probability[class_indices])
                    #     size1 = len(class_indices)
                    #
                    #     try:
                    #         pred_idx[size_pred:size_pred + class_len] = np.array(class_indices)[
                    ...
                    # both GMM and JS unicon

                    # pred_idx_gmm  = np.zeros(int(self.sample_ratio * num_sample))
                    # pred_idx_js = np.zeros(int(self.sample_ratio * num_sample))
                    # class_len = int(self.sample_ratio * num_sample/num_class)
                    # size_pred = 0
                    #
                    #
...
class cifar_dataloader():  
    ...
    def run(self, sample_ratio, mode, pred=[], prob=[], class_flex_adjust=[], gmm_prob=[]):
        if mode=='warmup':
            ...
            labeled_dataset = cifar_dataset(dataset=self.dataset, sample_ratio= sample_ratio, noise_mode=self.noise_mode, r=self.r, root_dir=self.root_dir, transform=self.transforms["labeled"], mode="labeled", noise_file=self.noise_file, pred=pred, probability=prob,log=self.log, class_flex_adjust=class_flex_adjust, gmm_prob=gmm_prob)
